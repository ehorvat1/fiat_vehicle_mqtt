...
function fiat_get_data($cfg) {
  ...
  foreach ($vins as $vin => $data) {
    $payload = array(
      ...
      'evinfo_unix_sec' => (floor($x['vehicle'][$vin]['status']['evInfo']['timestamp'] / 1000)), //EHorvat added this
      'status_timestamp' => epoche2time($x['vehicle'][$vin]['status']['timestamp'], 1000),
      'status_unix_sec' => (floor($x['vehicle'][$vin]['status']['timestamp'] / 1000)),  //EHorvat added this
      ...
      'vehicleinfo_unix_sec' => (floor($x['vehicle'][$vin]['status']['vehicleInfo']['timestamp'] / 1000)),   //EHorvat added this
      ...
      'location_unix_sec' => (floor($x['vehicle'][$vin]['location']['timeStamp'] / 1000)),   //EHorvat added this
